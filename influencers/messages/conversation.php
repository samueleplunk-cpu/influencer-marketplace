...
// Verifica che l'influencer abbia accesso alla conversazione
$stmt = $pdo->prepare("
    SELECT c.*, b.company_name as brand_name, b.logo, camp.title as campaign_title
...
$stmt = $pdo->prepare("
    UPDATE messages 
    SET is_read = TRUE 
    WHERE conversation_id = ? AND sender_type = 'brand' AND is_read = FALSE
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message']) && !empty(trim($_POST['message']))) {
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, 'influencer', ?)
        ");
        $stmt->execute([$conversation_id, $influencer_id, $message]);
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="d-flex align-items-center mb-4">
                ...
                <?php if (!empty($conversation['logo'])): ?>
                    <?php
                    $logo_filename = basename($conversation['logo']);
                    $logo_url = '/infl/uploads/brands/' . $logo_filename;
                    ?>
                    <img src="<?php echo htmlspecialchars($logo_url); ?>" 
                         class="rounded-circle me-3" 
                         style="width: 60px; height: 60px; object-fit: cover;" 
                         alt="<?php echo htmlspecialchars($conversation['brand_name']); ?>"
                         onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div class="rounded-circle bg-secondary d-flex align-items-center justify-content-center me-3" 
                         style="width: 60px; height: 60px; display: none;">
                        <span class="text-white"><?php echo substr($conversation['brand_name'], 0, 1); ?></span>
                    </div>
                <?php else: ?>
                    <div class="rounded-circle bg-secondary d-flex align-items-center justify-content-center me-3" 
                ...
                <div>
                    <h4 class="mb-1"><?php echo htmlspecialchars($conversation['brand_name']); ?></h4>
                ...
                <div class="card-body" style="max-height: 500px; overflow-y: auto;" id="messages-container">
                    ...
                    <?php else: ?>
                        <?php foreach ($messages as $msg): ?>
                            <div class="d-flex mb-3 <?php echo $msg['sender_type'] === 'influencer' ? 'justify-content-end' : 'justify-content-start'; ?>">
                                <div class="message-bubble <?php echo $msg['sender_type'] === 'influencer' ? 'bg-primary text-white' : 'bg-light'; ?>" 
                                     style="max-width: 70%; padding: 12px 16px; border-radius: 18px;">
                                    <div class="message-content">
                                        <?php echo nl2br(htmlspecialchars($msg['message'])); ?>
                                    </div>
                                    <div class="message-time small mt-1 <?php echo $msg['sender_type'] === 'influencer' ? 'text-white-50' : 'text-muted'; ?>">
                                        <?php echo date('d/m/Y H:i', strtotime($msg['created_at'])); ?>
                                        <?php if ($msg['sender_type'] === 'influencer' && $msg['is_read']): ?>
