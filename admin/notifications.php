...
<div class="row mt-4">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Statistiche Notifiche</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-3 text-center">
                        <div class="card bg-light">
                            <div class="card-body">
                                ...
                                ?>
                                <h3 class="text-primary"><?php echo $total_notifications; ?></h3>
                                <p class="mb-0">Notifiche Totali</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="card bg-light">
                            <div class="card-body">
                                <?php
                                $stmt = $pdo->prepare("SELECT COUNT(*) as unread FROM notifications WHERE is_read = FALSE");
                                $stmt->execute();
                                $unread_notifications = $stmt->fetch(PDO::FETCH_ASSOC)['unread'];
                                ?>
                                <h3 class="text-warning"><?php echo $unread_notifications; ?></h3>
                                <p class="mb-0">Non Lette</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="card bg-light">
                            <div class="card-body">
                                <?php
                                $stmt = $pdo->prepare("SELECT COUNT(DISTINCT user_id) as users FROM notifications");
                                $stmt->execute();
                                $active_users = $stmt->fetch(PDO::FETCH_ASSOC)['users'];
                                ?>
                    ...
                    <div class="col-md-3 text-center">
                        <div class="card bg-light">
                            <div class="card-body">
                                <?php
                                $stmt = $pdo->prepare("SELECT COUNT(*) as today FROM notifications WHERE DATE(created_at) = CURDATE()");
                                $stmt->execute();
                                $today_notifications = $stmt->fetch(PDO::FETCH_ASSOC)['today'];
                                ?>
                                <h3 class="text-info"><?php echo $today_notifications; ?></h3>
                                <p class="mb-0">Oggi</p>
                            </div>
                        </div>
                    </div>
